...
class EEG_Processing_User {
  ...
  private int beta = 2;
  private int gamma = 3;
  private String[] bandNames = {"θ","α","β","γ"};
...
//  final float[] min_allowed_peak_freq_Hz = {
//    3.0, 6.0, 10.0, 25.0
//  };  // low range for calculating background level
//  final float[] max_allowed_peak_freq_Hz = {
//    9.0, 14.0, 35.0, 57.5
//  };  // high range for calculating background level
...
//    helicopter = copter;
    ...
    for (int Ichan=0; Ichan<nchan; Ichan++){  // find the highest realative peak for each channel
      detectedPeak[Ichan]=new DetectedPeak();
    }
    int nBands = processing_band_low_Hz.length;
    peakPerBand = new DetectedPeak[nchan][nBands];
    for (int Ichan=0; Ichan<nchan; Ichan++){  // find the highest peak for each band in each channel
     for(int Ibands=0; Ibands<nBands; Ibands++){
       peakPerBand[Ichan][Ibands] = new DetectedPeak();
     }
    }
  }  // end of EEG_Processing_user
...
//
  ...
  FFT[] fftData) {              //holds the FFT (frequency spectrum) of the latest data
    ...
    for(int i=0; i<nchan; i++){
      ...
      if ((detectedPeak[i].freq_Hz >= processing_band_low_Hz[1]) && // <= 9Hz line 48
          (detectedPeak[i].freq_Hz < processing_band_high_Hz[1])) { // < 12Hz line 50
...
//          println("detectedPeak channel " + (i+1) + " = " + detectedPeak[i].SNR_dB);  // verbose debug
          ...
          if (isDetected) {
            //print some output
            println("EEG_Processing_User: peak!, Chan " + (i+1) + ", peak = " + detectedPeak[i].rms_uV_perBin + " uV at "
...
//      if (fftData != null) findBestFrequency_2DTraining(fftData, Ichan);
  ...
  void findPeakFrequency(FFT[] fftData, int Ichan) {
    ...
    for(int bandRange=0; bandRange<bandNames.length; bandRange++){
      peakPerBand[Ichan][bandRange].clear();
    }
    ...
    for(int bandRange=0; bandRange < bandNames.length; bandRange ++){
      ...
      if ((FFT_freq_Hz >= processing_band_low_Hz[bandRange]) && (FFT_freq_Hz <= processing_band_high_Hz[bandRange])) {
        ...
        if (FFT_value_uV > peakPerBand[Ichan][bandRange].rms_uV_perBin) {
          //this is bigger, so hold onto this value as the new "maximum"
          peakPerBand[Ichan][bandRange].bin  = Ibin;
          peakPerBand[Ichan][bandRange].freq_Hz = FFT_freq_Hz;
          peakPerBand[Ichan][bandRange].rms_uV_perBin = FFT_value_uV;
        }
   ...
   for (int bandRange=0; bandRange < bandNames.length; bandRange++){
    for (int Ibin=0; Ibin < nBins; Ibin++) {  // have to re arrange this so that it makes sense!
      FFT_freq_Hz = fftData[Ichan].indexToFreq(Ibin);

        if ((FFT_freq_Hz >= processing_band_low_Hz[bandRange]) && (FFT_freq_Hz <= processing_band_high_Hz[bandRange])) {
          if ((Ibin < peakPerBand[Ichan][bandRange].bin - 1) || (Ibin > peakPerBand[Ichan][bandRange].bin + 1)) {
...
//println(int(nBins));
    //compute mean
    peakPerBand[Ichan][bandRange].background_rms_uV_perBin = sqrt(sum_pow / count);

    //decide if peak is big enough to be detected
    peakPerBand[Ichan][bandRange].SNR_dB
      = 20.0f*(float)java.lang.Math.log10(peakPerBand[Ichan][bandRange].rms_uV_perBin
                                  / peakPerBand[Ichan][bandRange].background_rms_uV_perBin);
      }
  ...
  void updateGUI_FFTPlot(Blank2DTrace.PlotRenderer pr) {
    ...
    if (showDetectionOnGUI) {
      ...
      for (int Iband=theta; Iband<=gamma; Iband++) { // plot bands theta thru gamma
        ...
        // increase font size and print the greek letter?
        pr.canvas.text(bandNames[Iband], (int)x1, -(int)y1);  // print the band names in the FFT window
...
//println("alpha "+ x1 + " " + y1);
    ...
    for (int Ichan=0; Ichan < 8; Ichan++) {
      for(int Iband=0; Iband<bandNames.length; Iband++){
        //int Ichan = 2-1; //which channel to show on the GUI
        x1 = pr.valToX(processing_band_low_Hz[Iband]);  //starting coordinate, left
        x2 = pr.valToX(processing_band_high_Hz[Iband]);  //start coordinate, right
        y1 = pr.valToY(peakPerBand[Ichan][Iband].background_rms_uV_perBin * 129); //y-coordinate
...
//            float thresh_dB = peakPerBand[Ichan][Iband].threshold_dB;
//            float val_uV_perBin = peakPerBand[Ichan][Iband].background_rms_uV_perBin * sqrt(pow(10.0, thresh_dB/10.0));
...
//        } else {
          ...
          x1 = pr.valToX(peakPerBand[Ichan][Iband].freq_Hz);
          y1 = pr.valToY(peakPerBand[Ichan][Iband].rms_uV_perBin * 129); // *nBins
          is_detected = peakPerBand[Ichan][Iband].isDetected;
//print(Ichan + " ");
          addMarker(pr, x1, y1, is_detected, Ichan);
...
//    pr.canvas.stroke(0, 0, 0);  //black
    ...
    while (foo_x < x2+dx) {  //loop to make each dash
...
//       SOME COMPLICATED MACHINE LEARNING SHIT RIGHT HERE
//
